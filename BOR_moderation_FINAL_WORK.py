...
async def start_handler(message: Message):
    ...
    if CAPTCHA_ENABLED and user and user[9] == 0:
...
async def complete_task(callback: CallbackQuery, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        if task_type in ["manual", "photo", "screenshot"]:
            ...
            await state.set_state(ManualSubmissionState.content)
            await callback.message.answer(
                "📤 Отправьте фото/скрин доказательства"
            )
            return
...
async def delete_task_list(callback: CallbackQuery):
    ...
    for task in tasks:
        kb.row(
            InlineKeyboardButton(
                text=f"❌ {task[1]}",
                callback_data=f"delete_task_confirm:{task[0]}"
            )
        )
...
async def admin_ref_reward(callback: CallbackQuery):
    await callback.message.answer(
        "Введите новую награду за реферала"
    )


@dp.message(F.text.regexp(r"^ref\s+\d+(\.\d+)?$"))
async def change_ref_reward(message: Message):
    ...
    await message.answer(
        f"✅ Новая реферальная награда: {amount}"
    )
...
@dp.message(ManualSubmissionState.content, F.photo)
async def submit_photo(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        cur=await db.execute(
            "INSERT INTO task_completions(user_id,task_id,proof,status,created_at) VALUES(?,?,?,?,?)",
            (message.from_user.id,task_id,photo_id,"pending",datetime.now().isoformat())
        )
        completion_id=cur.lastrowid
        await db.commit()

    kb=InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="✅ Подтвердить",callback_data=f"approve:{completion_id}"),
        InlineKeyboardButton(text="❌ Отклонить",callback_data=f"reject:{completion_id}")
    ]])
...
async def approve_task(callback: CallbackQuery):
    try:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            cursor = await db.execute(
                """
                SELECT tc.user_id, tc.task_id, t.reward
            ...
            user_id, task_id, reward = row

            # Проверка чтобы не начислялось повторно
            cursor = await db.execute(
        ...
        with suppress(Exception):
            await bot.send_message(
                user_id,
                f"✅ Ваше выполнение задания #{task_id} подтверждено!\n💰 Начислено: {reward}"
            )

        # Обновление сообщения в админке
        with suppress(Exception):
...
async def reject_task(callback: CallbackQuery):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute(
            "SELECT user_id, task_id FROM task_completions WHERE id = ?",
            (completion_id,)
        )
        row = await cursor.fetchone()
    ...
    with suppress(Exception):
        await bot.send_message(
            user_id,
            f"❌ Ваше выполнение задания #{task_id} отклонено администратором"
        )
